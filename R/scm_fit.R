...
#'   predictor matrix for SCM (see Abadie et al. 2010, §2.3). Each [pred()]
...
#'   §3.2: the pre-treatment window is split in half; V is selected to minimise
#'   MSPE on the validation half, then W is refit on the full window.
#' @param donor_mspe_threshold Donor pool filtering threshold (Abadie 2021 §4).
...
#'   `"bfgs"` uses R's L-BFGS-B, which requires only O(k²) inner QP calls and
